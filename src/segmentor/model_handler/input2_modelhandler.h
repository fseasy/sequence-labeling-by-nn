...
public :
    ...
    void read_training_data_and_build_dynamic_and_postag_dicts(std::istream &is, 
                                                               std::vector<IndexSeq> &dynamic_sents, std::vector<IndexSeq> &fixed_sents,
                                                               std::vector<IndexSeq> &tag_seqs);
...
{
    i2m->get_input_dict_wrapper().set_threshold(freq_thres, prob_thres);
}
                                                            ...
                                                            std::vector<IndexSeq> &tag_seqs)
{
    ...
    IndexSeq dsent,
        ...
        tag_seq;
    // pre-allocation
    tmp_dyamic_sents.reserve(MaxSentNum); // 2^19 =  480k pairs 
    ...
    while (getline(is, line)) {
        ...
        while( iss >> words_line )
        {
            ...
            {
                Index dword_id = word_dict_wrapper.Convert(tmp_word_cont[i]) ;
                                                                     ...
                                                                     std::vector<IndexSeq> &tag_seqs)
{
    ...
    DictWrapper &word_dict_wrapper = i2m->get_input_dict_wrapper() ;
                                                 ...
                                                 std::vector<IndexSeq> &fsents)
{
    ...
    while( getline(is, line) )
    {
        ...
        for( size_t i = 0 ; i < raw_sent.size() ; ++i )
        {
            ...
            fsent.push_bakc(fword_dict.Convert(raw_sent[i]);
        }
              ...
              // devel will creat another Computation Graph , so we need to create new scoce to release it before devel .
                ...
                for( size_t word_idx = 0; word_idx < sent.size(); ++word_idx )
                {
                    dynamic_sent_after_replace_unk[word_idx] =
                        word_dict_wrapper.ConvertProbability(dsent.at(word_idx));
                }
...
template <typename I2Model>
float Input2ModelHandler<I2Model>::devel(const std::vector<IndexSeq> *p_sents, 
                                              const std::vector<IndexSeq> *p_tag_seqs)
{
    unsigned nr_samples = p_sents->size();
    ...
    for (unsigned access_idx = 0; access_idx < nr_samples; ++access_idx)
    {
        ...
        i2m->predict(cg, sent,predict_tag_seq);
...
void Input2ModelHandler<I2Model>::predict(std::istream &is, std::ostream &os)
{
    ...
    for (unsigned int i = 0; i < raw_instances.size(); ++i)
    {
        ...
        cnn::ComputationGraph cg;
        i2m->predict(cg, sent, pred_tag_seq);
        Seq words ;
