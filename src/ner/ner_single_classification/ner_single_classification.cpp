...
#include "ner/model_handler/input2D_modelhandler.h"
...
int train_process(int argc, char *argv[], const string &program_name)
{
    ...
    op_des.add_options()
        ...
        ("conlleval_script_path" , po::value<string>() , "The path to conlleval script for NER")
        ...
        ("postag_embedding_dim",po::value<unsigned>()->default_value(5), "The dimension for postagger embedding.")
        ("nr_lstm_stacked_layer", po::value<unsigned>()->default_value(1), "The number of stacked layers in bi-LSTM.")
        ("lstm_x_dim" , po::value<unsigned>()->default_value(50) , "The dimnesion for lstm X.")
    ...
    string conlleval_script_path, *p_conlleval_script_path ;
    if( devel_data_path == "" ) p_conlleval_script_path = nullptr ;
    else
    {
        ...
        p_conlleval_script_path = &conlleval_script_path ;
    }
    ...
    vector<IndexSeq> sents ,
        postag_seqs,
        ner_seqs;
    model_handler.read_training_data_and_build_dicts(train_is, sents , postag_seqs, ner_seqs);
    train_is.close();
