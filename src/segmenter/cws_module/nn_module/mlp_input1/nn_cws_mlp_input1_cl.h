#ifndef SLNN_SEGMENTER_CWS_MODULE_NN_MODULE_CWS_MLP_INPUT1_CL_H_
#define SLNN_SEGMENTER_CWS_MODULE_NN_MODULE_CWS_MLP_INPUT1_CL_H_
...
{
    this->word_expr_layer.reset(new Index2ExprLayer(this->get_dynet_model(), param.corpus_token_dict_size,
        param.corpus_token_embedding_dim));
    this->window_expr_generate_layer.reset(new WindowExprGenerateLayer(this->get_dynet_model(), param.window_size,
        param.corpus_token_embedding_dim));
    std::vector<unsigned> mlp_hidden_dim_list;
    this->mlp_hidden_layer.reset(new MLPHiddenLayer(this->get_dynet_model(), param.mlp_input_dim,
        param.mlp_hidden_dim_list, param.mlp_dropout_rate, 
        utils::get_nonlinear_function_from_name(param.mlp_nonlinear_function_str)));
    this->output_layer.reset(new CWSSimpleBareOutput(this->get_dynet_model(), param.mlp_hidden_dim_list.back(),
        param.output_dim));
}
