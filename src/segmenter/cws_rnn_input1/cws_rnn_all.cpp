...
using slnn::segmenter::rnn_input1::RnnAll;

static const string PROGRAM_HEADER = "segmenter Rnn-input1-all based on DyNet Library";
...
int train_process(int argc, char *argv[], const string &program_name)
{
    ...
    model_op.add_options()
        ...
        ("nr_stack_layer", po::value<unsigned>()->default_value(1), "The number for bi-rnn stacked layer.")
        ("h_dim", po::value<unsigned>()->default_value(100), "The dimension for bi-rnn hidden layer.")
        ("dropout_rate", po::value<float>(), "droupout rate for training (bi-rnn output)")
    ...
    std::shared_ptr<RnnAll>  ra = RnnAll::create_new_model(argc, argv, rng_seed);
    ...
    ra->set_model_structure_param_from_outer(var_map);
    ...
    ra->get_token_module()->build_lexicon_if_necessary(train_is);

    vector<RnnAll::AnnotatedDataProcessedT> training_data;
    modelhandler::read_training_data(train_is, *ra, training_data);
    train_is.close();

    ra->finish_read_training_data();
    
    // build model structure
    ra->build_model_structure();

    // reading developing data
    vector<RnnAll::AnnotatedDataProcessedT> devel_data;
    std::ifstream devel_is(devel_data_path);
    if (!devel_is) {
        fatal_error("Error : failed to open devel file: `" + devel_data_path + "`");
    }
    modelhandler::read_devel_data(devel_is, *ra, devel_data);
    devel_is.close();
    // Train
    auto devel_record_list = modelhandler::train(*ra, training_data, devel_data, opts);

    // save model
    ra->save_model(model_os);
    model_os.close();
    // save log
    try
    {
...
int devel_process(int argc, char *argv[], const string &program_name)
{
    ...
    std::shared_ptr<RnnAll> ra = RnnAll::load_and_build_model(model_is, dynet_argc, dynet_argv_ptr);
    model_is.close();
    ...
    // devel
    modelhandler::devel(*ra, devel_data); 
    return 0;
}
...
int predict_process(int argc, char *argv[], const string &program_name)
{
    ...
    shared_ptr<RnnAll> ra = RnnAll::load_and_build_model(is, dynet_argc, dynet_argv_ptr);
    is.close();
    ...
    if ("" == output_path)
    {
        modelhandler::predict(*ra, raw_is, cout); // using `cout` as output stream 
        raw_is.close();
    }
    else
    {
        ...
        modelhandler::predict(*ra, raw_is, os);
        os.close();
    }
    return 0;
}
