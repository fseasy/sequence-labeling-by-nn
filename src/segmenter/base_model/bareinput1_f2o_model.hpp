#ifndef SLNN_SEGMENTER_BASE_MODEL_BAREINPUT1_F2O_MODEL_0628_HPP_
#define SLNN_SEGMENTER_BASE_MODEL_BAREINPUT1_F2O_MODEL_0628_HPP_
...
template <typename RNNDerived>
void CWSBareInput1F2OModel<RNNDerived>::set_model_param_from_outer(const boost::program_options::variables_map &var_map)
{
    ...
    this->cws_feature.set_feature_parameters(start_here_embedding_dim, pass_here_embedding_dim, end_here_embedding_dim,
        ...
        chartype_embedding_dim);
    softmax_layer_input_dim = rnn_h_dim * 2 + this->cws_feature.get_feature_dim();

}

template <typename RNNDerived>
void CWSBareInput1F2OModel<RNNDerived>::set_model_param_from_inner()
{
...
template<typename RNNDerived>
dynet::expr::Expression CWSBareInput1F2OModel<RNNDerived>::build_loss(dynet::ComputationGraph &cg,
    const IndexSeq &input_seq, 
    ...
    const IndexSeq &gold_seq)
{
    ...
    return output_layer->build_output_loss(std::vector<std::vector<dynet::expr::Expression> *>({
           &l2r_exprs, &r2l_exprs, &feature_exprs}), gold_seq) ;
}

template<typename RNNDerived>
void CWSBareInput1F2OModel<RNNDerived>::predict(dynet::ComputationGraph &cg,
    const IndexSeq &input_seq,
    const CWSFeatureDataSeq &feature_seq,
    IndexSeq &pred_seq)
{
