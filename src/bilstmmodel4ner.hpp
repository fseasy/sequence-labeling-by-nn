...
struct BILSTMModel4NER
{
    ...
    Parameters *input_affine_word_w_param;
    Parameters *input_affine_postag_w_param;
    Parameters *input_affine_b_param;
    Parameters *input_SOS_param; // using param 
    Parameters *input_EOS_param;
    ...
    // model structure : using const !
    
    unsigned WORD_DICT_SIZE; // extra `WORD_UNK`
    unsigned WORD_EMBEDDING_DIM;

    unsigned POSTAG_DICT_SIZE; // extra `POSTAG_UNK`
    ...
    **/
    ...
    void build_model_structure(bool is_print_model_info = true)
    {
        ...
        m = new Model();
        l2r_builder = new LSTMBuilder(LSTM_LAYER, LSTM_X_DIM, LSTM_H_DIM, m);
        r2l_builder = new LSTMBuilder(LSTM_LAYER, LSTM_X_DIM, LSTM_H_DIM, m);
        ...
        input_affine_b_param = m->add_parameters({ LSTM_X_DIM });
        input_SOS_param = m->add_parameters({ LSTM_X_DIM });
        input_EOS_param = m->add_parameters({ LSTM_X_DIM });

        l2r_tag_hidden_w_param = m->add_parameters({ NER_LAYER_HIDDEN_DIM , LSTM_H_DIM });
        r2l_tag_hidden_w_param = m->add_parameters({ NER_LAYER_HIDDEN_DIM , LSTM_H_DIM });
        pretag_tag_hidden_w_param = m->add_parameters({ NER_LAYER_HIDDEN_DIM  , NER_TAG_EMBEDDING_DIM }); // ADD for PRE_TAG
        tag_hidden_b_param = m->add_parameters({ NER_LAYER_HIDDEN_DIM });
        pretag_SOS_param = m->add_parameters({ NER_TAG_EMBEDDING_DIM });

        tag_output_w_param = m->add_parameters({ NER_LAYER_OUTPUT_DIM , NER_LAYER_HIDDEN_DIM }); 
        tag_output_b_param = m->add_parameters({ NER_LAYER_OUTPUT_DIM });
        ...
        ComputationGraph *p_cg, Stat *p_stat = nullptr)
    {
        ...
        // - input 
        Expression input_affine_word_w_exp = parameter(cg, input_affine_word_w_param);
        Expression input_affine_postag_w_exp = parameter(cg, input_affine_postag_w_param);
        Expression input_affine_b_exp = parameter(cg, input_affine_b_param);
        Expression input_SOS_exp = parameter(cg, input_SOS_param);
        Expression input_EOS_exp = parameter(cg, input_EOS_param);
        ...
        for (unsigned i = 0; i < sent_len; ++i)
        {
            ...
            Expression input_affine_exp = affine_transform({input_affine_b_exp, input_affine_word_w_exp, word_lookup_exp,
                input_affine_postag_w_exp, postag_lookup_exp }); 
            input_affine_exp_cont[i] = noise(input_affine_exp, 0.1f);
        }


        // 2. left 2 right , calc Expression of every timestep of LSTM
        l2r_builder->add_input(input_SOS_exp);
        for (unsigned i = 0; i < sent_len; ++i) 
        {
            l2r_lstm_output_exp_cont[i] = l2r_builder->add_input(input_affine_exp_cont[i]);
        }

        // 3. right 2 left , calc Expression of every timestep of LSTM
        r2l_builder->add_input(input_EOS_exp);
        for (int i = static_cast<int>(sent_len) - 1; i >= 0; --i) 
        {
            // should be int , or never stop
            r2l_lstm_output_exp_cont[i] = r2l_builder->add_input(input_affine_exp_cont[i]);
        }
    ...
    void do_predict(const IndexSeq *p_sent, const IndexSeq *p_postag_seq, IndexSeq *p_predict_tag_seq , ComputationGraph *p_cg )
    {
        ...
        for (unsigned i = 0; i < sent_len; ++i)
        {
            Expression word_lookup_exp = lookup(cg, words_lookup_param, p_sent->at(i));
            Expression postag_lookup_exp = lookup(cg, postags_lookup_param, p_postag_seq->at(i));
            input_affine_exp_cont[i] = affine_transform({ input_affine_b_exp, input_affine_word_w_exp, word_lookup_exp,
                input_affine_postag_w_exp, postag_lookup_exp });
        }
