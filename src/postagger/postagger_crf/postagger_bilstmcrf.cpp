...
const string PROGRAM_DESCRIPTION = "Postagger-BILSTM_CRF based on DyNet Library";
...
int train_process(int argc, char *argv[], const string &program_name)
{
    ...
    op_des.add_options()
        ...
        ("merge_hidden_dim", po::value<unsigned>()->default_value(32), "The dimension for tag hidden layer.")
    ...
    vector<IndexSeq> sents , 
        postag_seqs ;
    model_handler.read_training_data_and_build_word_and_postag_dicts(train_is, sents ,  postag_seqs);
    train_is.close();
    ...
    if ("" != devel_data_path)
    {
        ...
        model_handler.read_devel_data(devel_is, dev_sents , dev_postag_seqs);
        devel_is.close();
    ...
    {
        p_dev_sents = p_dev_postag_seqs = nullptr;
    }

    // Train 
    model_handler.train(&sents ,&postag_seqs , max_epoch, 
        p_dev_sents , p_dev_postag_seqs ,
    ...
    if (0 == var_map.count("model"))
    {
        ...
        ostringstream oss;
        oss << "dc_" << dc_model.word_embedding_dim << "_" << dc_model.lstm_h_dim
...
int devel_process(int argc, char *argv[], const string &program_name)
{
    ...
    vector<IndexSeq> sents,
        postag_seqs;
    model_handler.read_devel_data(devel_is, sents , postag_seqs);
    devel_is.close();

    // devel
    model_handler.devel(&sents , &postag_seqs, p_error_output_os); // Get the same result , it is OK .
    if (p_error_output_os)
    {
