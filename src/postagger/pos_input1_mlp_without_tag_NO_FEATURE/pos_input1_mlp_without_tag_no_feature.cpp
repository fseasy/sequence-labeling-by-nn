...
#include "postagger/model_handler/input1_mlp_modelhandler_no_feature.h"
...
int train_process(int argc, char *argv[], const string &program_name)
{
    ...
    Input1MLPModelNoFeatureHandler<Input1MLPWithoutTagNoFeatureModel> model_handler;
    ...
    model_handler.read_training_data(train_is, sents ,context_feature_gp_seqs, tag_seqs);
    train_is.close();
    ...
    model_handler.read_devel_data(devel_is, dev_sents, dev_context_feature_gp_seqs, dev_tag_seqs);
    devel_is.close();

    // Train 
    model_handler.train(sents, context_feature_gp_seqs,  tag_seqs , 
        max_epoch, 
        dev_sents , dev_context_feature_gp_seqs, dev_tag_seqs , 
        devel_freq , 
        trivial_report_freq);
...
int devel_process(int argc, char *argv[], const string &program_name)
{
    ...
    vector<IndexSeq> sents,
        tag_seqs ;
    vector<ContextFeatureDataSeq> context_feature_gp_seqs;
    model_handler.read_devel_data(devel_is, sents, context_feature_gp_seqs, tag_seqs);
    devel_is.close();

    // devel
    model_handler.devel(sents , context_feature_gp_seqs, tag_seqs); 
