...
template <typename MLPModel>
void Input1MLPModelHandler<MLPModel>::train(const std::vector<IndexSeq> *p_sents,
    const std::vector<POSContextFeature::ContextFeatureIndexGroupSeq> *p_context_feature_gp_seqs,
    const std::vector<POSFeature::POSFeatureIndexGroupSeq> *p_feature_gp_seqs,
    const std::vector<IndexSeq> *p_tag_seqs,
    unsigned max_epoch,
    ...
    unsigned trivial_report_freq)
{
    ...
    for( unsigned nr_epoch = 0; nr_epoch < max_epoch && is_train_ok; ++nr_epoch )
    {
        ...
        for( unsigned i = 0; i < nr_samples; ++i )
        {
            ...
            {
                float acc = devel(p_dev_sents, p_dev_context_feature_gp_seqs, p_dev_feature_gp_seqs, p_dev_tag_seqs);
...
template <typename MLPModel>
float Input1MLPModelHandler<MLPModel>::devel(const std::vector<IndexSeq> *p_sents,
    const std::vector<POSContextFeature::ContextFeatureIndexGroupSeq> *p_context_feature_gp_seqs,
    const std::vector<POSFeature::POSFeatureIndexGroupSeq> *p_feature_gp_seqs,
    const std::vector<IndexSeq> *p_tag_seqs)
{
