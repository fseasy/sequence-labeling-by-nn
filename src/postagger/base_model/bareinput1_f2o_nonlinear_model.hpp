...
void BareInput1F2ONonlinearModel<RNNDerived>::set_model_param(const boost::program_options::variables_map &var_map)
{
    ...
    if( nonlinear_func_name == "rectify" ){ pos_feature_hidden_layer_nonlinear_func = &dynet::expr::rectify; }
    else if( nonlinear_func_name == "tanh" ){ pos_feature_hidden_layer_nonlinear_func = &dynet::expr::tanh; }
...
template<typename RNNDerived>
dynet::expr::Expression BareInput1F2ONonlinearModel<RNNDerived>::build_loss(dynet::ComputationGraph &cg,
    const IndexSeq &input_seq, 
    const POSFeature::POSFeatureIndexGroupSeq &features_gp_seq,
    const IndexSeq &gold_seq)
{
    ...
    return this->output_layer->build_output_loss(std::vector<std::vector<dynet::expr::Expression> *>({ &l2r_exprs, &r2l_exprs, &feature_exprs }),
        gold_seq) ;
}

template<typename RNNDerived>
void BareInput1F2ONonlinearModel<RNNDerived>::predict(dynet::ComputationGraph &cg,
    const IndexSeq &input_seq,
    const POSFeature::POSFeatureIndexGroupSeq &features_gp_seq,
    IndexSeq &pred_seq)
{
    ...
    this->output_layer->build_output(std::vector<std::vector<dynet::expr::Expression> *>({ &l2r_exprs, &r2l_exprs, &feature_exprs }),
        pred_seq) ;
}
