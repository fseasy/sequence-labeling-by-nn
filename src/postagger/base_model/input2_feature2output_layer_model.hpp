#ifndef POS_BASE_MODEL_INPUT2_FEATURE2OUTPUT_LAYER_MODEL_HPP_
#define POS_BASE_MODEL_INPUT2_FEATURE2OUTPUT_LAYER_MODEL_HPP_
...
class Input2F2OModel : public Input2WithFeatureModel<RNNDerived>
{
    friend class boost::serialization::access;

public:
    Input2F2OModel();
    virtual ~Input2F2OModel();
    Input2F2OModel(const Input2F2OModel &) = delete;
    Input2F2OModel& operator()(const Input2F2OModel&) = delete;
...
template<typename RNNDerived>
Input2F2OModel<RNNDerived>::Input2F2OModel() 
...
template <typename RNNDerived>
void Input2F2OModel<RNNDerived>::set_model_param(const boost::program_options::variables_map &var_map)
{
...
template <typename RNNDerived>
void Input2F2OModel<RNNDerived>::build_fixed_dict(std::ifstream &is)
{
...
template <typename RNNDerived>
void Input2F2OModel<RNNDerived>::load_fixed_embedding(std::ifstream &is)
{
...
template<typename RNNDerived>
dynet::expr::Expression Input2F2OModel<RNNDerived>::build_loss(dynet::ComputationGraph &cg,
    const IndexSeq &dynamic_sent,
    const IndexSeq &fixed_sent,
    const POSFeature::POSFeatureIndexGroupSeq &features_gp_seq,
...
template<typename RNNDerived>
void Input2F2OModel<RNNDerived>::predict(dynet::ComputationGraph &cg,
    const IndexSeq &dynamic_sent,
    const IndexSeq &fixed_sent,
    const POSFeature::POSFeatureIndexGroupSeq &features_gp_seq,
    IndexSeq &pred_seq)
...
void Input2F2OModel<RNNDerived>::save(Archive &ar, const unsigned version) const
{
...
template <typename RNNDerived>template< typename Archive>
void Input2F2OModel<RNNDerived>::load(Archive &ar, const unsigned version)
{
    ar & dynamic_word_dict_size & dynamic_word_embedding_dim
        & fixed_word_dict_size & fixed_word_embedding_dim
        & rnn_x_dim & rnn_h_dim & nr_rnn_stacked_layer
...
template <typename RNNDerived>
template<typename Archive>
void Input2F2OModel<RNNDerived>::serialize(Archive & ar, const unsigned version)
{
