...
const std::string BILSTMCRFDCModelHandler::number_transform_str = "##";
const size_t BILSTMCRFDCModelHandler::length_transform_str = number_transform_str.length();

BILSTMCRFDCModelHandler::BILSTMCRFDCModelHandler(BILSTMCRFDCModel4POSTAG &dc_m) 
    :dc_m(dc_m) , best_acc(0.f) , best_model_tmp_ss()
{}

void BILSTMCRFDCModelHandler::build_fixed_dict_from_word2vec_file(std::ifstream &is)
{
...
void BILSTMCRFDCModelHandler::set_unk_replace_threshold(int freq_thres, float prob_thres)
{
    dc_m.dynamic_dict_wrapper.set_threshold(freq_thres, prob_thres);
}

void BILSTMCRFDCModelHandler::do_read_annotated_dataset(istream &is, vector<IndexSeq> &dynamic_sents, vector<IndexSeq> &fixed_sents,
...
void BILSTMCRFDCModelHandler::read_training_data_and_build_dynamic_and_postag_dicts(istream &is, vector<IndexSeq> &dynamic_sents, vector<IndexSeq> &fixed_sents,
    vector<IndexSeq> &postag_seqs)
{
...
void BILSTMCRFDCModelHandler::read_devel_data(istream &is, vector<IndexSeq> &dynamic_sents, vector<IndexSeq> &fixed_sents,
    vector<IndexSeq> &postag_seqs)
{
...
void BILSTMCRFDCModelHandler::read_test_data(istream &is, vector<Seq> &raw_test_sents, vector<IndexSeq> &dynamic_sents,
...
void BILSTMCRFDCModelHandler::finish_read_training_data(boost::program_options::variables_map &varmap)
{
    // set param 
...
void BILSTMCRFDCModelHandler::build_model()
{
    dc_m.build_model_structure();
    dc_m.print_model_info();
}

void BILSTMCRFDCModelHandler::load_fixed_embedding(std::istream &is)
{
...
void BILSTMCRFDCModelHandler::train(const vector<IndexSeq> *p_dynamic_sents, const vector<IndexSeq> *p_fixed_sents,
    ...
    unsigned verbose_train_report )
{
    ...
    for (unsigned nr_epoch = 0; nr_epoch < max_epoch; ++nr_epoch)
    {
        ...
        for (unsigned i = 0; i < nr_samples; ++i)
        {
            ...
            auto loss_expr = dc_m.viterbi_train(cg, &dynamic_sent_after_replace_unk, p_fixed_sent, p_tag_seq,
...
float BILSTMCRFDCModelHandler::devel(const std::vector<IndexSeq> *p_dynamic_sents, const std::vector<IndexSeq> *p_fixed_sents,
...
void BILSTMCRFDCModelHandler::predict(std::istream &is, std::ostream &os)
{
...
void BILSTMCRFDCModelHandler::save_model(std::ostream &os)
{
...
void BILSTMCRFDCModelHandler::load_model(std::istream &is)
{
